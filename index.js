...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.cy19tcg.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        ...
        const reviewCollection = client.db('manufacturer_toolboxes').collection('reviews');
        const paymentCollection = client.db('manufacturer_toolboxes').collection('payments');
        ...
        app.put('/user/:email', async (req, res) => {
            ...
            const token = jwt.sign({ email: email }, process.env.ACCCESS_TOKEN_SECRET)
        ...
        app.patch('/order/:id', verifyJWT, async (req, res) => {
            ...
            const updatedQTY = await toolCollection.updateOne(toolFilter, updatedQuantity);
